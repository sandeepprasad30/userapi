...
describe("# Users", () => {
    ...
    //let token;
    let username='saprasa2';

    it("should retrieve a user", () => {
      return request.get(endpoint + "/getuser/" + username)
        .set("username", username)
        .set("Accept", "application/json")
        .expect("Content-Type", /json/)
        .expect(res => res.body.result.UserName.should.equal(username))
        .expect(200);
    });
    ...
    it("should retrieve a user", () => {
        return request.get(endpoint + "/getuser/" + userId)
            .set("Authorization", token)
            .set("Accept", "application/json")
            .expect("Content-Type", /json/)
